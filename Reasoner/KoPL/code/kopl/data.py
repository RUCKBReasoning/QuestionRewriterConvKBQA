...
class KB(object):
	def __init__(self, kb):
		...
		def simple_el(ent_ids):
			max_freq = 0
			target_ent = ent_ids[0]
			for ent_id in ent_ids:
				# print(ent_id)
				if len(self.entities[ent_id]['relations']) > max_freq:
					target_ent = ent_id
					max_freq = len(self.entities[ent_id]['relations'])
			assert target_ent != ''
			return target_ent
		print('3')
		self.name_to_id = {k: [simple_el(list(v))] for k, v in tqdm(self.name_to_id.items())}
